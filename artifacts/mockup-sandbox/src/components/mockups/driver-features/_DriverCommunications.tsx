...
const SAMPLE_DATA: Communications = {
  safety: { stopped: true, reason: 'Coach is stopped. GPS speed 0 mph.' },
  reports: [],
  instructions: [{
    id: 'sample-instruction',
    message: 'Hold at Maple Avenue for two connecting passengers. Dispatch will confirm when clear to depart.',
    createdAt: '2025-09-23T14:08:00-04:00',
    deliveredAt: '2025-09-23T14:08:04-04:00',
    acknowledgedAt: null,
  }],
};
...
export function DriverCommunications({
  ...
  const [data, setData] = useState<Communications | null>(SAMPLE_DATA);
  const [error, setError] = useState('');
  const [busy, setBusy] = useState('');

  const request = useCallback(async <T,>(path: string, init?: RequestInit) => {
    if (path === '/driver/communications') return SAMPLE_DATA as T;
    if (path.includes('/acknowledge') && init?.method === 'POST') {
      setData(current => current ? {
        ...current,
        instructions: current.instructions.map(item => ({ ...item, acknowledgedAt: new Date().toISOString() })),
      } : current);
    }
    return {} as T;
  }, []);
