...
const metroPort = Number(process.env.EXPO_BUILD_METRO_PORT || 19001);
...
async function startMetro(expoPublicDomain, expoPublicReplId) {
  ...
  const env = {
    ...process.env,
    // Publishing needs a headless local bundler, not Expo's interactive/network checks.
    CI: '1',
    EXPO_OFFLINE: '1',
  ...
  metroProcess = spawn(
    'pnpm',
    ['exec', 'expo', 'start', '--no-dev', '--minify', '--localhost', '--max-workers', '2', '--port', String(metroPort)],
    {
  ...
  metroProcess.once('error', (error) => { startupError = error.message; });
  metroProcess.once('exit', (code, signal) => {
    startupError = `Metro exited before becoming ready (code ${code}, signal ${signal})`;
  });
  ...
  // Cold publishing containers can need more than a minute to load the workspace.
  const startupDeadline = Date.now() + 180_000;
  let nextProgress = Date.now() + 15_000;
  while (Date.now() < startupDeadline) {
    if (startupError) exitWithError(startupError);
    ...
    if (Date.now() >= nextProgress) {
      console.log('Waiting for Metro startup...');
      nextProgress = Date.now() + 15_000;
    }
  }

  exitWithError(`Metro startup timeout after 180 seconds at ${metroOrigin}/status. See Metro output above.`);
}
